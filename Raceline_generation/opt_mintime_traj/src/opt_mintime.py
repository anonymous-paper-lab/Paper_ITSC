...
# questa funzione l'ho messa io per far funzionare il codice
import casadi as ca
def convert_to_float_list(values):
    return [float(v) if isinstance(v, ca.MX) or isinstance(v, ca.DM) else v for v in values]
...
def opt_mintime(reftrack: np.ndarray,
    ...
    for k in range(N):
        ...
        w.append(Uk)
        lbw.append(convert_to_float_list([delta_min, f_drive_min, f_brake_min, gamma_y_min]))
        ubw.append(convert_to_float_list([delta_max, f_drive_max, f_brake_max, gamma_y_max]))
        w0.append(convert_to_float_list([0.0] * nu))
        ...
        for j in range(d):
            ...
            Xc.append(Xkj)
            w.append(Xkj)
            lbw.append(convert_to_float_list([-np.inf] * nx))
            ubw.append(convert_to_float_list([np.inf] * nx))
            if pars["pwr_params_mintime"]["pwr_behavior"]:
                w0.append(convert_to_float_list([v_guess, 0.0, 0.0, 0.0, 0.0,
                           machine.temp_guess, batt.temp_guess, inverter.temp_guess,
                           radiators.temp_cool_mi_guess, radiators.temp_cool_b_guess,
    ...
    # # DEBUG: Diagnosi degli elementi in lbw
    # for i, val in enumerate(lbw):
    #     try:
    #         print(f"[DEBUG] lbw[{i}] - shape: {np.shape(val)} | type: {type(val)}")
    #     except Exception as e:
    #         print(f" Errore con lbw[{i}]: {e}")
    #         print(f"    Contenuto grezzo: {val}")

    # # Ora esegui il concatenamento (se è tutto corretto)
